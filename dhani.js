...
fake = setting.Fake
yt = setting.yt
const fakeimage = fs.readFileSync ('./media/bg.jpg')
const fakedhani = fs.readFileSync ('./media/bg.jpg')
...
module.exports = Dhani = async (Dhani, mek, _welkom) => {
	try {
		...
		const ownerNumber = [`${owner}@s.whatsapp.net`, `6283142717817@s.whatsapp.net`]
...
case 'store':
               list = []
               listmenu = [`dmff`,`dmml`,`ucpubg`,`cod`,`payment`]
               listmenuu = [`ᴅɪᴀᴍᴏɴᴅ ғʀᴇᴇ ғɪʀᴇ`,`ᴅɪᴀᴍᴏɴᴅ ᴍᴏʙɪʟᴇ ʟᴇɢᴇɴᴅ`,`ᴜᴄ ᴘᴜʙɢ`,`ᴄᴀʟʟ ᴏғ ᴅᴜᴛʏ ᴍᴏʙɪʟᴇ`,`ᴘᴀʏᴍᴇɴᴛ`]
               nombor = 1
               startnum = 0
...
case 'codm':
case 'cod':
gambar = fs.readFileSync('./media/cod.jpg')
...
case 'berber':
menu =`*${ucapanWaktu} @${sender.split('@')[0]}*
*Admin siap membantu rekber !!*

Jika mau rekber, GOBER langsung
Saja buat grup / MC KAN`
break
//══════════[ Fitur Owner ]══════════//
